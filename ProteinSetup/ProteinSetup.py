...
from pCore             import logFile, LogFileActive, Selection, Clone, TextFileWriter
...
class ProteinSetup (object):
        ...
        for atomName in atomNames:
          if residue.childIndex.has_key (atomName):
            ...
            if LogFileActive (log):
              log.Text ("\nWarning: atom name %s not found in residue %s\n" % (atomName, resName))
  ...
  def WriteList (self, filename, toWrite):
    """Write a list containing integers to a file."""
    outfile = TextFileWriter (filename)

    outfile.Open ()
    outfile.file.writelines (" ".join (map (str, toWrite)))
    outfile.Close ()
  ...
  def WriteQuantum (self, system, filestem = "qc"):
    ...
    if self.isInitialized:
      ...
      self.WriteList (filestem, self.indicesQuantum)
  ...
  def WriteFlexible (self, system, filestem = "flex"):
    ...
    if self.isInitialized:
      ...
      self.WriteList (filestem, self.indicesFlexible)
  ...
  def WriteRestrained (self, system, filestem = "restr"):
    ...
    if self.isInitialized:
      ...
      self.WriteList (filestem, self.indicesRestrained)
